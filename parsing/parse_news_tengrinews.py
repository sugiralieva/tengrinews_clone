...
images_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'img', 'images_news')
...
class UpdateNews:
    ...
    def insert_data_to_table_news(self, title, news_text, publication_date, image):
        with sq.connect(db_path) as conn:
            cursor = conn.cursor()
    ...
    def parse_news_pages(self, list_news_urls):
        for news_url in list_news_urls:
            ...
            if time_ >= d: # сравниваем даты, если дата сегодня, записываем в БД
                ...
                self.insert_data_to_table_news(title, news_text, time_, img_name)
