...
def create_table_kazakhstan_future():
...
def insert_data_to_table_kazakhstan_future(title, news_text, publication_date, image):
    with sq.connect('../instance/tengrinews.db') as conn:
        cursor = conn.cursor()
...
def download(url):
    ...
    r = open("..\\static\\img\\images_kazakhstan_future\\" + url.split("/")[-1], "wb")
...
for news_url in list_news_urls:
    ...
    download(url_img)
    create_table_kazakhstan_future()
    insert_data_to_table_kazakhstan_future(title, news_text, time_, img_name)
